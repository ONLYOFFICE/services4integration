...
sed -i "$((`grep -n 'http {' /etc/nginx/nginx.conf | awk -F':' '{print $1}'`+1))i\ \ \ \ include       /tmp/nginx_conf/map.conf;" /etc/nginx/nginx.conf
sed -i "$((`grep -n 'server {' /etc/nginx/nginx.conf | awk -F':' '{print $1}'`+1))i\ \ \ \ \ \ \ \ include       /tmp/nginx_conf/local.conf;" /etc/nginx/nginx.conf
